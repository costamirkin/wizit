...
public class SalesItemizer extends ItemizedIconOverlay<OverlayItem> {
	protected Context mContext;
	private Vector<NewSale> mSalesVector;

	public SalesItemizer(final Context context, final List<OverlayItem> aList,
			Vector<NewSale> salesVector) {
	...
	protected boolean onSingleTapUpHelper(final int index,
		...
		dialog.setPositiveButton("To their site", new OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				Intent siteIntent = new Intent(Intent.ACTION_VIEW, Uri.parse(""
						+ mSalesVector.get(index).getmSite()));
	...
	@Override
	public boolean onSingleTapUp(MotionEvent e, MapView mapView) // get the
																	// position
																	// that has
																	// tapped
	{
